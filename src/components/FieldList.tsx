                  ...
                  <div className="w-full border border-border bg-bg/40 p-1.5 mt-1">
                    ...
                    <div className="flex gap-1">
                      <button className="btn-sm btn-active flex-1 text-[10px]" title="Dessiner une nouvelle parcelle dans ce champ"
                        onClick={(e) => { e.stopPropagation(); useAppStore.getState().setDrawForChampId(champ.id); useAppStore.getState().setDrawTarget('field'); useAppStore.getState().setStatus(`DESSIN PARCELLE pour "${champ.name}" — cliquez les sommets`); useAppStore.getState().setSidebarOpen(false); useAppStore.getState().setMobileRightOpen(false) }}>
                        ▭ Dessiner
                      </button>
                  ...
                  <div className="w-full border border-border bg-bg/40 p-1.5 mt-1">
                    ...
                    <div className="flex gap-1">
                      <button className="btn-sm btn-active flex-1 text-[10px]" title="Dessiner une nouvelle parcelle"
                        onClick={(e) => { e.stopPropagation(); useAppStore.getState().setDrawForChampId(champ.id); useAppStore.getState().setDrawTarget('field'); useAppStore.getState().setStatus(`DESSIN PARCELLE pour serre "${champ.name}"`); useAppStore.getState().setSidebarOpen(false); useAppStore.getState().setMobileRightOpen(false) }}>
                        ▭ Dessiner
                      </button>
                      <button className="btn-sm btn-cyan flex-1 text-[10px]" title="Assigner des parcelles existantes"
                        onClick={(e) => { e.stopPropagation(); if (!assigning) setSelectedParcelleIds([]); setAssigning(!assigning) }}>⊕ Existantes</button>
